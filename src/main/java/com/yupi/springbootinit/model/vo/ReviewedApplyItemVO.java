...
/**
 * “我已审核”列表展示模型：
 * - 左侧：申请单当时的快照信息（字段名与 SiteApplyDO 对齐，便于前端复用）
 * - 右侧：当前主表（OS_SITE）的实时信息及存在性标签
 */
@Data
@AllArgsConstructor
@NoArgsConstructor
@Builder
@ApiModel("ReviewedApplyItemVO")
public class ReviewedApplyItemVO {

    // -------------------- 申请侧（历史快照，与 SiteApplyDO 字段对齐） --------------------
    ...
    @ApiModelProperty("申请审核时间（通过/驳回时间）")
    private LocalDateTime reviewedAt;

    // 可选：与 SiteApplyVO 对齐的主覆盖国家中文/英文名（方便前端直接用同一组件）
